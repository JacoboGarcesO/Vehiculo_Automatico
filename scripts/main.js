        ...
        this.btn_1ra.addEventListener('click', () => {
            if (_1ra == false&&clutch) {
                ...
                if (marcha && clutch) {
                    ...
                    velocidades.push((contadorTotalKM+10) / (contadorSeg / 3600));
                };
            };
        });
        this.btn_2da.addEventListener('click', () => {
            if (_2da == false&&clutch) {
                _2da = true;
                _1ra = false;
                _3ra = false;
                _4ta = false;
                ...
                if (marcha && clutch) {
                    ...
                    velocidades.push((contadorTotalKM+20) / (contadorSeg / 3600));
                };
            };
        });
        this.btn_3ra.addEventListener('click', () => {
            if (_3ra == false&&clutch) {
                _3ra = true;
                _1ra = false;
                _2da = false;
                _4ta = false;
                ...
                if (marcha && clutch) {
                    ...
                    velocidades.push((contadorTotalKM+30) / (contadorSeg / 3600));
                };
            };
        });
        this.btn_4ta.addEventListener('click', () => {
            if (_4ta == false&&clutch) {
                _4ta = true;
                _1ra = false;
                _3ra = false;
                _2da = false;
                ...
                if (marcha && clutch) {
                    ...
                    velocidades.push((contadorTotalKM+40) / (contadorSeg / 3600));
                };
            };
        });
        this.btn_5ta.addEventListener('click', () => {
            if (_5ta == false&&clutch) {
                _5ta = true;
                _1ra = false;
                _3ra = false;
                _2da = false;
                ...
                if (marcha && clutch) {
                    ...
                    velocidades.push((contadorTotalKM+50) / (contadorSeg / 3600));
                };
            };
        });
        this.btn_6ta.addEventListener('click', () => {
            if (_6ta == false&&clutch) {
                _6ta = true;
                _1ra = false;
                _3ra = false;
                _2da = false;
                ...
                if (marcha && clutch) {
                    ...
                    velocidades.push((contadorTotalKM+60) / (contadorSeg / 3600));
                };
            };
        });
    ...
    encender() {
        this.btn_Encendido.addEventListener('click', () => {
            if (clutch == true && neutro == true && freno_Mano == true) {
                ...
                marcha=false;
                if (contadorTotalKM >= distancia && acelerar == false && freno_Mano &&neutro) {
                    encendido = false;
                    let sumaVEL = 0;
                    ...
                    document.getElementById('Alerts').innerHTML = `<div class="bg-primary alert alert-warning alert-dismissible fade show" role="alert">
            ...
            if (encendido && clutch == true && freno_Pie == true && freno_Mano == false && _1ra == true) {
                ...
                this.btn_Acelerar.addEventListener('click', () => {
                    if (encendido && freno_Pie == false && acelerar == true && clutch == false) {
                        marcha = true;
                        interval = setInterval("actualizar()", 2000);
                        contadorSeg += getRandomInt(700, 800);
                        velocidades.push((contadorTotalKM+5) / (contadorSeg / 3600));
                    };
                });
            };
        });
...
let contadorSeg = 0;
function actualizar() {
    if (contadorTotalKM == distancia + 1) {
        clearInterval(interval);
        document.getElementById('Alerts').innerHTML = `<div class="bg-success alert alert-warning alert-dismissible fade show" role="alert">
        <strong class='text-body'>
        ¡Llegó al destino, ya puede apagar el carro!
        </strong>
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>`;
    } else {
        if (cont == 0) {
            cont = 20;
            document.getElementById('Alerts').innerHTML = `
            <div class="bg-success alert alert-warning alert-dismissible fade show" role="alert">
            <strong class='text-body'>
               Ya puede hacer el cambio si lo cree conveniente (Recuerde presionar el clutch)
                </strong>
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>`;
        } else {
            ...
            <div class="bg-success alert alert-warning alert-dismissible fade show" role="alert">
            <strong class='text-body'>
                Distancia óptima que falta para el cambio: ${cont}Km <br>
